...
from PyQt5.QtWidgets import (
    ...
    QDialog, QFormLayout, QComboBox, QMessageBox,QFontComboBox, QFontDialog, QAbstractButton, QFrame
)
...
# Basic logging setup: logs to terminal and to thunderbird_debug.log
...
# Global exception hook so uncaught exceptions are logged nicely
import sys
import traceback
...
class ChatbotGUI(QWidget):
    ...
    def style_button(self, button, color="#007ACC", text_color=None):
        """Styles a button-like widget. For QRadioButton we apply a simpler
        readable text color that contrasts with the current theme. For
        QPushButton we keep the existing visual treatment.
        """ 
        ...
        # Decide contrasting text color for radio buttons if not explicitly set
        if cls_name == 'QRadioButton':
            if text_color is None:
                # Prefer white on dark themes, black on light/system themes
                text_color = "#FFFFFF" if getattr(self, 'current_theme', 'system') == 'dark' else "#000000"
            # Use transparent background and clear font weight for radio labels
            button.setStyleSheet(f"QRadioButton {{ color: {text_color}; background-color: transparent; font-weight: bold; }}")
            logger.debug(f"Styled QRadioButton '{getattr(button, 'text', lambda: '')()}' with color {text_color}")
            return

        # Default styling for push buttons
        if text_color is None:
            text_color = "white"
        ...
        while not self.logged_in_email:
            ...
            def handle_dev_login():
                ...
                def attempt_dev_login():
                    logger.debug(f"Developer login attempt: authname={username_input.text()}")
                    if username_input.text() == "Thunderbird" and password_input.text() == "Thunderbird123":
                        logger.info("Developer mode credentials accepted.")
                        ...
                        logger.debug("Developer login failed")
            ...
            def forgot_password_popup():
                ...
                def send_code():
                    ...
                    logger.debug(f"Forgot password: send_code to {email}")
                    if not email or '@' not in email:
                        QMessageBox.warning(popup, "Hata", "Geçersiz e-posta adresi giriniz.")
                        logger.debug("send_code aborted: invalid email")
                        return
                    ...
                    try:
                        ...
                        logger.debug("send_code succeeded (simulation)")
                ...
                def do_reset():
                    ...
                    if not code_verified[0]:
                        status_label.setText("Önce e-posta doğrulaması yapmalısınız.")
                        logger.debug("do_reset aborted: code not verified")
                        return
                    email = email_input.text().strip()
                    new_pass = new_pass_input.text()
                    new_pass2 = new_pass2_input.text()
                    if not email or '@' not in email:
                        ...
                        logger.debug("do_reset failed: invalid email")
                        return
                    if not new_pass or not new_pass2:
                        status_label.setText("Yeni şifre alanı boş olamaz.")
                        logger.debug("do_reset failed: empty new password")
                        return
                    if new_pass != new_pass2:
                        status_label.setText("Şifreler eşleşmiyor.")
                        logger.debug("do_reset failed: password mismatch")
                        return
                    conn = sqlite3.connect("memory.db")
                    cursor = conn.cursor()
                    ...
                    if not cursor.fetchone():
                        ...
                        conn.close()
                        logger.debug("do_reset failed: no such user")
                        return
            ...
            # Enter key in first password field triggers registration (connected below after definition)
            ...
            def attempt_login():
                ...
                try:
                    ...
                    if not row:
                        ...
                        self.shimmer_label(login_status_label)
                        conn.close()
                        logger.debug("Login failed: no such account")
                        return
            ...
            def attempt_register():
                ...
                logger.debug(f"Attempting registration for: {email}")
                ...
                if cursor.fetchone():
                    ...
                    conn.close()
                    logger.debug("Registration failed: email already exists")
                    return
                conn.close()
                ...
                for _ in range(3):
                    ...
                    self.shimmer_label(reg_status_label)
                    logger.debug("Registration failed: verification failed")
                    return
                try:
                    conn = sqlite3.connect("memory.db")
                    cursor = conn.cursor()
            ...
            # Also allow pressing Enter in password fields to submit the registration
            reg_password_input.returnPressed.connect(attempt_register)
            reg_password2_input.returnPressed.connect(attempt_register)
    ...
    def open_settings_panel(self):
        ...
        self.style_button(rb_light, color="#273bae")      
        self.style_button(rb_dark, color="#273bae")
        self.style_button(rb_system, color="#273bae")
        for rb in (rb_light, rb_dark, rb_system):
            theme_group.addButton(rb)
            t_layout.addWidget(rb)

        # Pre‑select current theme and capture original for revert behaviour
        ...
        # Immediate preview when user selects a radio (no DB write)
        rb_light.toggled.connect(lambda checked: checked and self._apply_theme_styles('light'))
        rb_dark.toggled.connect(lambda checked: checked and self._apply_theme_styles('dark'))
        rb_system.toggled.connect(lambda checked: checked and self._apply_theme_styles('system'))
        ...
        def save_and_close():
            # Persist the currently previewed theme
        ...
        save_theme_btn.clicked.connect(save_and_close)

        # When the settings overlay is closed without saving, revert preview
        def close_settings_revert():
            if not saved['value']:
                try:
                    self._apply_theme_styles(original_theme)
                except Exception:
                    logger.exception('Failed to revert theme on close')
            splash.close()

        back_btn.clicked.connect(close_settings_revert)
        # Also revert when the dialog is dismissed by other means (ESC, close)
        splash.rejected.connect(close_settings_revert)
        ...
        back_btn.setToolTip("Kapat")
        # Close the entire settings overlay when pressed
        back_btn.clicked.connect(lambda: splash.close())
    ...
    def open_theme_selector(self):
        ...
        # make sure radio labels contrast with currently applied theme
        self.style_button(light_theme_btn)
        self.style_button(dark_theme_btn)
        self.style_button(system_theme_btn)
    ...
    def apply_theme(self, theme):
        ...
        # Persist the theme selection and re-style widgets.
        # Use helper to apply the visual styles first then persist to DB.
        ...
        try:
            ...
            conn.commit()
            conn.close()
            logger.info(f"Theme persisted: {theme}")
        except Exception:
            logger.exception("Failed to persist theme change")

    def _apply_theme_styles(self, theme):
        """Apply visual theme styles without persisting to the DB.
        This is used for immediate preview on radio selection.
        """
        if theme == "dark":
            base = "background-color: #121212; color: #E0E0E0;"
            radio_color = "#FFFFFF"
        elif theme == "light":
            base = "background-color: #FFFFFF; color: #000000;"
            radio_color = "#000000"
        else:
            base = ""
            radio_color = "#000000"
        # Apply stylesheet for base + radio color
        self.setStyleSheet(base + f" QRadioButton {{ color: {radio_color}; background-color: transparent; }}")
        # Track current theme for other helpers
        self.current_theme = theme
        logger.debug(f"Preview theme applied: {theme}")
        # Re-apply per-widget styling so widgets created earlier (e.g. radio labels) update
        try:
            self.refresh_theme_widgets()
        except Exception:
            logger.exception("Failed to refresh themed widgets during preview")

    def refresh_theme_widgets(self):
        """Re-apply styling to widgets that need explicit re-styling when theme changes."""
        from PyQt5.QtWidgets import QRadioButton, QPushButton
        # Re-style radio buttons so their labels contrast properly
        try:
            for rb in self.findChildren(QRadioButton):
                try:
                    self.style_button(rb)
                except Exception:
                    logger.exception("Failed to style radio button")
            # Optionally re-style push buttons to ensure consistent look
            for btn in self.findChildren(QPushButton):
                try:
                    self.style_button(btn)
                except Exception:
                    logger.exception("Failed to style push button")
        except Exception:
            logger.exception("refresh_theme_widgets top-level failure")
        conn = sqlite3.connect("memory.db")
        cursor = conn.cursor()
        cursor.execute("UPDATE user_profile SET theme=? WHERE email=?", (theme, self.logged_in_email))
        conn.commit()
    ...
    def get_user_profile(self, email=None):
        ...
        conn.close()
        # Safely extract fields by name to be robust across schema changes.
        if not result:
            return {
                "username": "default",
                "language": "tr",
        ...
        # Helper to get column value or default
        def col_val(name, default=""):
            if name in cols:
                try:
                    return result[cols.index(name)]
                except Exception:
                    return default
            return default

        birthdate = col_val('birthdate', '') or ''
        age = 0
        if birthdate:
            try:
        ...
        return {
            "username": col_val('username', 'default'),
            "language": col_val('language', 'tr'),
            "tone": col_val('tone', 'formal'),
            "interests": col_val('interests', ''),
            "sources": col_val('sources', 'json,wikipedia,chatgpt'),
            "email": col_val('email', ''),
            "first_name": col_val('first_name', ''),
            "last_name": col_val('last_name', ''),
            "birthdate": birthdate,
            "age": age,
            "chat_count": col_val('chat_count', 0) or 0,
            "theme": col_val('theme', 'system') or 'system'
        }
...
class DevGUI(QWidget):
    ...
    def open_theme_selector(self):
        ...
        # Ensure radio labels are readable according to the current theme
        self.style_button(light_theme_btn)
        self.style_button(dark_theme_btn)
        self.style_button(system_theme_btn)
